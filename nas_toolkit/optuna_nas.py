...
from models import MLP  # Import baseline model
...
class OptimizedMLP(nn.Module):
    """可配置的 MLP 模型，用于架构搜索"""
    def __init__(self, trial, input_dim=5):
        super(OptimizedMLP, self).__init__()
        
        # 架构搜索参数
        self.n_layers = trial.suggest_int('n_layers', 3, 8)  # 网络深度
        self.hidden_dim = trial.suggest_categorical('hidden_dim', [64, 128, 256, 512])  # 隐藏层维度
        self.dropout_rate = trial.suggest_float('dropout_rate', 0.0, 0.5)  # Dropout 率
        self.activation = trial.suggest_categorical('activation', ['relu', 'leaky_relu', 'tanh', 'elu'])  # 激活函数
        self.use_batch_norm = trial.suggest_categorical('use_batch_norm', [True, False])  # 是否使用 BatchNorm
        self.init_method = trial.suggest_categorical('init_method', ['he', 'xavier', 'orthogonal', 'normal'])  # 初始化方法
        self.skip_connections = trial.suggest_categorical('skip_connections', [True, False])  # 是否使用跳跃连接
        ...
        # 隐藏层
        for i in range(self.n_layers - 2):  # -2 因为已有输入层，还需要输出层
...
def periodic_progress_update():
    """Periodically update progress and create visualizations"""
    while True:
        try:
...
def run_two_stage_nas(architecture_trials=30, hyperparams_trials=50, storage_url=None):
    ...
    # 创建最终结果可视化
    create_progress_plot('final_nas_results.png')
    logging.info("📊 最终结果图表已保存: final_nas_results.png")
